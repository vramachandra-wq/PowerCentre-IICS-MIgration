...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "IDMC export package generation, or all metadata, validation, remediation, and automation outputs."
        ),
    )
    parser.add_argument(
        "--config",
    ...
    parser.add_argument(
        "--remediated-folder",
        default=None,
        help="Folder containing remediated XML files for --mode idmc-export. Defaults to output/remediated_xml.",
    )
    parser.add_argument(
        "--export-package-name",
        default="Custom_Project_Export.zip",
        help="ZIP filename generated by --mode idmc-export.",
    )
    parser.add_argument(
        "--idmc-export-strategy",
        choices=["POWERCENTER_XML_TASK", "CLOUD_NATIVE_CONVERSION", "IICS_API_DEPLOYMENT"],
        default="POWERCENTER_XML_TASK",
        help=(
            "Execution path represented in the generated IDMC package. PowerCenter XML cannot be uploaded "
            "directly as a native CDI mapping."
        ),
    )
    parser.add_argument(
        "--reference-export-package",
        default=None,
        help="Known-good IDMC export ZIP used as the internal template for selectable asset artifacts.",
    )
...
def main() -> None:
    ...
    elif args.mode == "idmc-export":
        from business.export.idmc_export_package import generate_idmc_export_package

        summary = generate_idmc_export_package(
            config=config,
            logger=logger,
            remediated_folder=args.remediated_folder,
            package_name=args.export_package_name,
            execution_strategy=args.idmc_export_strategy,
            reference_package=args.reference_export_package,
        )
        logger.info("IDMC export package generation completed. %s", summary)
    else:
        from business.parser.xml_parser import XMLParser
